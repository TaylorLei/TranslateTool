                ...
                for out in layout:
                    ...
                    if isinstance(out,LTTextBoxHorizontal):
                        ...
                        if content:
                            ...
                            to_trans_content11 = content.replace(".\n",".$$$$$   ")
                            to_trans_content12 = to_trans_content11.replace(". \n", ".$$$$$   ")
                            to_trans_content13 = to_trans_content12.replace("\n", " ")
                            to_trans_content = to_trans_content13.replace(".$$$$$   ", ".\n\n")


                            #ret = translate_func(to_trans_content)
                            ret = translate_func1(to_trans_content)
                            trans = ret if ret else '翻译失败'
                            ret2 = translate_func2(to_trans_content)
                            trans22 = ret2 if ret2 else '翻译失败'


                            self.write("\n\n\n$$$$$ 原始文档 *>>> \n")
                            self.write(content)
                            self.write("\n\n && 调整顺序后文档 *>>>*>>> \n\n")
                            self.write(to_trans_content)
                            #self.write(to_trans_content13)
